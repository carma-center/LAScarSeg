...
::computeKappa(long ix, long iy, long iz)
{
  ...
  float tmp = mp_phi->GetPixel(idx);

  /**
     test
  if (isnan(tmp))
    {
      douher::writeImage<float, 3>(this->mp_phi, "mp_phi.nrrd");
      raise(SIGABRT);
    }
  */
  ...
  if (xok)
    {
      float tmp1 = mp_phi->GetPixel(idx1);
      float tmp2 = mp_phi->GetPixel(idx2);
  ...
  if (yok)
    {
      float tmp3 = mp_phi->GetPixel(idx3);
      float tmp4 = mp_phi->GetPixel(idx4);
  ...
  if (zok)
    {
      float tmp5 = mp_phi->GetPixel(idx5);
      float tmp6 = mp_phi->GetPixel(idx6);
  ...
  /**
   * The k above is NOT kappa (mean curvature) itself. In fact it is
